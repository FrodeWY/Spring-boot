 ...
 *
 * 如果一个容器有多个实现BeanFactoryPostProcessor的接口，这时候就需要实现类实现org.springframework.core.Ordered接口，
 * 设置order属性来保证自定义的BeanFactoryPostProcessor的实现类的执行顺序。
 *
 */
@Component
public class MyBeanFactoryPostProcessor2 implements BeanFactoryPostProcessor,Ordered {
  ...
  public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
    ...
    if (propertyValues.contains("name")) {
      propertyValues.addPropertyValue(new PropertyValue("name", "titi2"));
    }
  }
